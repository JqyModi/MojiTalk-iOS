...
- (void)loadLive2D {
    ...
    // 尝试加载live 2d model
    if ([[L2DCacheManager sharedInstance] getL2DModelCacheWithModelName:self.configurationModel.fileName] == nil) {
        
        L2DModel *model = new L2DModel();
        model->LoadAssets(modelPath, modelJsonName);
        [[L2DCacheManager sharedInstance] setL2DModelToCache:model withModelName:self.configurationModel.fileName];
    }
    
    // 加载背景图像
    if ([self.configurationModel hasBackgroundImage]) {
        TextureInfo* backgroundTexture = [L2DTextureHelper createTextureFromPngFile:[self.configurationModel.backgroundImagePath UTF8String]];
        ...
        _back = [[L2DSprite alloc] initWithMyVar:x Y:y Width:displaySize.width Height:displaySize.height MaxWidth:self.frame.size.width MaxHeight:self.frame.size.height Texture:backgroundTexture->id];
    }
}
