...
- `Bridge/`: 本地编写的 Objective-C 桥接层 (L2DCubism, OpenMTKView 等)
- `OpenLive2D.podspec`: 本地开发 Pod 配置文件
...
4. **Swift 调用层**：通过 CocoaPods 自动生成的 Module 直接 `import OpenLive2D` 调用。

## 遇到的问题与解决方案 (Troubleshooting)

### 1. 编译错误：`fatal error: 'new' file not found`
- **问题原因**：Swift 在尝试导入 `OpenLive2D` 模块时，Clang 扫描器扫描到了 SDK 的 C++ 头文件（如 `CubismFramework.hpp`）。由于 Swift 默认不支持 C++ Interop，导致编译器无法处理标准库包含。
- **解决方案**：在 `OpenLive2D.podspec` 中精确设置 `s.public_header_files`。仅公开 `L2DCubism.h`, `OpenMTKView.h` 等纯 OC 接口。将所有 `.hpp` 和其他内部头文件设为私有（Project Headers）。
...
## 如何验证编译与运行
1. 打开 `OpenTalk.xcworkspace`。
2. 选择 `OpenTalk` Scheme。
3. 选择真机或模拟器。
4. 执行 `Product -> Build`。
5. 运行查看 Live2D 模型加载情况。

## 注意事项
- 如果修改了 `OpenLive2D.podspec` 的结构（如增删文件、修改公开头文件范围），必须在终端执行 `pod install` 以同步工程配置。
