...
struct Live2DView: UIViewRepresentable {
    func makeUIView(context: Context) -> OpenMTKView {
        Live2DController.shared.isLoaded = false
        // Load target model (defaulting to suzu)
        let config = OpenL2DFileManager.L2DModelType.suzu.toConfigurationModel()
        
        let view = OpenMTKView(configurationModel: config)
    ...
    func updateUIView(_ uiView: OpenMTKView, context: Context) {
        // Reactive updates
    }
    
    static func dismantleUIView(_ uiView: OpenMTKView, coordinator: ()) {
