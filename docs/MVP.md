...
### 2.3 业务框架
- [x] **极简登录**：支持游客模式进入，或简化的手机号/账号登录。
- [x] **单场景模式**：默认提供一个万用对话场景，暂不实现复杂的场景切换系统。

### 2.4 合规与上架必备 (App Store Store Compliance)
必须满足 Apple 审核底线要求：
- **账号注销 (Account Deletion)**：App 内必须提供永久删除账号的入口 (Guideline 5.1.1(v))。
- **用户协议与隐私 (Legal)**：登录页或显著位置展示 EULA 与 Privacy Policy 链接。
- **权限规范**：麦克风等隐私权限必须包含具体用途描述。
