...
### 12. 用户安全与引导
- [x] **[Critical]** 账号注销二次确认弹窗 ✅
  - [x] 在 `UserProfileView` 的"永久注销账户"按钮添加 Alert 确认
  - [x] 警告文案：已本地化支持多语言
  - [x] 提供"取消"和"确认注销"两个选项

- [x] **[Critical]** 对话页面首次使用引导 ✅
  - [x] 实现蒙版式功能引导 (Onboarding Overlay)
  - [x] 引导内容：
    - [x] 点击消息气泡播放 TTS
    - [x] 长按消息呼出翻译/语法分析菜单
    - [x] 语音输入按钮使用方法
    - [x] Live2D 角色交互说明
  - [x] 使用 UserDefaults 记录是否已展示过引导

- [x] **[High]** 登录页增强 ✅
  - [x] 添加累计练习次数滚动数字展示 (参考截图设计)
    - [x] 实现数字滚动动画效果
    - [x] 数据来源：可以是固定数值或从后端获取全局统计
  - [x] 添加帮助入口按钮 (右上角耳机图标)
...
### 13. 国际化支持 (i18n) ✅
- [x] **[High]** 配置多语言框架 ✅
  - [x] 创建 `Localizable.xcstrings` 资源文件
  - [x] 配置支持语言：简体中文、英文、日文、韩文
  - [x] 创建 `LocalizedString.swift` 类型安全访问层
...
- [x] **[High]** 翻译所有 UI 文案 ✅
  ...
  - [x] 对话页 (ChatView)
    - [x] 输入框占位符、菜单项、错误提示
  - [x] 设置页 (UserProfileView)
    - [x] 设置项标题、描述、按钮文案
  - [x] 引导页 (OnboardingOverlayView)
    - [x] 所有步骤标题和描述
  - [x] 登录帮助 (LoginHelpView)
    - [x] FAQ 问答、联系客服
  - [x] 登录增强组件 (LoginEnhancementViews)
    - [x] 滚动数字、欢迎消息
